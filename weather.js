...
function createParticles(count, weatherType) {
  ...
  const resizeCanvas = () => {
    ...
    ctx.scale(dpr, dpr); // Scale the context for high-DPI displays
  };
...
function getParticleColor(weatherType) {
  switch (weatherType) {
    case 'rainy':
...
function setBackgroundByWeather(icon) {
  ...
  // Reinitialize particles for the new weather type
  createParticles(100, weatherType); // Adjust particle count as needed
}
...
function initApp() {
  ...
  createParticles(100, 'clear'); // Example: Start with clear weather particles
